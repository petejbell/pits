                    ...
                    if (access(filename, F_OK ) == -1)
                    {
                        ...
                        if ((fp = fopen(filename, "wt")) != NULL)
                        {
                            if (Channel == 4)
                            {
                                ...
                                // Check if altitude is high enough to take video and start every 5 minutes (if have GPS lock)
                                Minutes = GPS->Seconds / 60;
                                if ((GPS->Altitude > 10000) && ((Minutes % 2) == 0) && (GPS->Satellites > 4))
                                {
                                    fprintf(fp, "raspivid -t 30000 -b 17000000 -fps 30 -o %s/$2/$1.h264\n sleep 30\n", Config.Channels[Channel].SSDVFolder);
                                }
